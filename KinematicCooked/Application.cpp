...
int Applicataion::Execute()
{
    ...
    m_KinematicModel->Create(0.0f, 1.0f, 4.0f);
...
bool Applicataion::SDLInit()
{
    ...
    m_SdlWindow = SDL_CreateWindow("PhysX Samples - KinematicCooked", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 800, 600, window_flags);
...
void Applicataion::CalculateFramesPerSecond()
{
    ...
    if (time > 1.0f)
    {
        ...
        auto title = "PhysX Samples - KinematicCooked - FPS: " + std::to_string(fps) + " (" + std::to_string(1000.0f / fps) + " ms)";
