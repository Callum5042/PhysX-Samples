...
int Applicataion::Execute()
{
    ...
    m_DynamicLockedModel = std::make_unique<DX::DynamicLockedModel>(m_DxRenderer.get(), m_Physics.get());
    m_DynamicLockedModel->Create(0.0f, 1.0f, 4.0f);
    ...
    while (e.type != SDL_QUIT)
    {
        ...
        else
        {
            ...
            m_DynamicLockedModel->Update();
            m_DxShader->UpdateWorldBuffer(m_DynamicLockedModel->World, m_DynamicLockedModel->Colour);
            m_DynamicLockedModel->Render();
...
bool Applicataion::SDLInit()
{
    ...
    m_SdlWindow = SDL_CreateWindow("PhysX Samples - DynamicLockedAxis", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 800, 600, window_flags);
...
void Applicataion::CalculateFramesPerSecond()
{
    ...
    if (time > 1.0f)
    {
        ...
        auto title = "PhysX Samples - DynamicLockedAxis - FPS: " + std::to_string(fps) + " (" + std::to_string(1000.0f / fps) + " ms)";
